In this page, I managed to display some 3Dmol instance using a third-party JavaScript library [3Dmol.js](https://3dmol.org/doc/index.html ) [^1] , this is an excellent library for online molecular visualization on webpage(Shout out to the developers in  University of Pittsburgh!). Here I will show some cases of embedding proteins or molecules via setting 3Dmol parameters within HTML tags.
For practical embedding 3Dmol in your code, check the tutorial  [Using 3Dmol within your code](https://3dmol.org/doc/tutorial-code.html ). All the local pdb files displayed in this page can be found [here](https://github.com/JianyuZhao612/jianyuzhao612.github.io/tree/master/docs/scripts/attachments).

- #### Display PDB proteins from remote database

Here is 2POR protein displayed. After configuration of the class as `viewer_3Dmoljs` in `div`, and the parameter `data-pdb` as `2POR`, 3dmol.js will download the corresponding PDB file via an HTTP request from RCSB PDB API（In this case, [https://files.rcsb.org/download/2POR.pdb](https://files.rcsb.org/download/2POR.pdb) ）, parse the file and render the final protein on the canvas.  More settings on display style please check  [embedding a 3Dmol Viewer](https://3dmol.org/doc/tutorial-embeddable.html ).

<div style="height: 700px; width: 700px; position: relative;" class='viewer_3Dmoljs' data-pdb='2POR' data-backgroundcolor='0xffffff' data-style='stick' data-ui='true'></div> 
<script>setInterval((function(){$3Dmol.viewers&&$3Dmol.viewers[0]&&$3Dmol.viewers[0].rotate(1)}),50)</script>
